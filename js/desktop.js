...
$(document).on('click', "a[data-carro='1']", function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.result === "ok") {
                alert(data.response.canastaHTML);
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.result === "ok") {
                ...
                bootbox.alert(
                    "<div data-role='main'><div class='ui-content' data-role='content' role='main'>" + data.response + "<a class='ui-btn ui-btn-r ui-corner-all ui-shadow' data-rel='back' href='#'>Aceptar</a></div></div>"
                );
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
