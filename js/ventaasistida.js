...
function buscarProductos(text, obj, request) {
    $.ajax({
        type: 'POST',
        async: true,
        url: request + '/puntoventa/venta/ventaasistida/pedido/buscarProducto',
        data: {busqueda: text},
        beforeSend: function () {
...
$(document).on('click', 'a[data-action="asignar-cliente"]', function () {
    $.ajax({
        type: 'POST',
        dataType: 'json',
        async: true,
...
$(document).on('click', 'a[data-action="asignar-pdv-direccion"]', function () {
    $.ajax({
        ...
        dataType: 'json',
        async: true,
        url: requestUrl + '/puntoventa/venta/ventaasistida/pedido/asignarpdv',
...
$(document).on('click', "a[data-cargar-nacional='1'], a[data-cargar-nacional='2']", function () {
    ...
    if (tipo == 1) {
        var producto = $(this).attr('data-producto');
        var cantidadU = $('#cantidad-producto-unidad-' + producto + "_" + pdv).val();
        cantidadU = parseInt(cantidadU);
        if (isNaN(cantidadU)) {
            cantidadU = -1;
        }
        var cantidadF = parseInt($('#cantidad-producto-fraccion-' + producto + "_" + pdv).val());
    ...
    $.ajax({
        ...
        dataType: 'json',
        async: true,
        url: requestUrl + '/puntoventa/venta/ventaasistida/carro/agregar',
        data: data,
        beforeSend: function () {
            Loading.show();
        },
...
$(document).on('click', "button[data-role='pdvgeodireccion']", function () {
    $.ajax({
        ...
        dataType: 'json',
        async: true,
        url: requestUrl + '/puntoventa/venta/ventaasistida/pedido/geoDireccion',
...
$(document).on('click', "button[data-role='disminuir-cantidad']", function() {
    ...
    var pdv = $(this).attr('data-pdv');
    var nro = $("#cantidad-producto-unidad-" + codigoProducto+"_"+pdv ).val();
    nro--;
    if (nro < 0) {
        nro = 0;
    }
    $("#subtotal-producto-unidad-" + codigoProducto+"_"+pdv).html("$" + format(nro * valorUnidad));
    $("#cantidad-producto-unidad-" + codigoProducto +"_"+pdv).val(nro);
});
...
$(document).on('change', "select[data-role='orden-listaproductos']", function() {
  $.ajax({
      ...
      dataType: 'json',
      async: true,
      url: requestUrl + '/puntoventa/venta/ventaasistida/catalogo/filtrar',
...
function modificarCarro(position, modificar, maximo) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        async: true,
        url: requestUrl + '/puntoventa/venta/ventaasistida/carro/modificar',
        data: data,
        beforeSend: function() {
            Loading.show();
        },
...
function disminuirCantidadFraccionado(codigoProducto, numeroFracciones, unidadFraccionamiento, valorFraccionado, valorUnidad, idUnico, pdv) {
    var nro = $("#cantidad-producto-fraccion-" + codigoProducto +"_"+pdv).val();
    nro--;
    if (nro < 0) {
        var nro = $("#cantidad-producto-unidad-" + codigoProducto +"_"+pdv ).val();
        nro--;
    ...
    $("#subtotal-producto-fraccion-" + codigoProducto +"_"+pdv).html("$" + format(nro * valorFraccionado));
    $("#cantidad-producto-fraccion-" + codigoProducto +"_"+pdv).val(nro);
}

function aumentarCantidadFraccionado(codigoProducto, numeroFracciones, unidadFraccionamiento, valorFraccionado, valorUnidad, idUnico,pdv) {
    var nro = $("#cantidad-producto-fraccion-" + codigoProducto +"_"+pdv).val();
    nro++;
...
function validarCantidadFraccionado(codigoProducto, numeroFracciones, unidadFraccionamiento, valorFraccionado, valorUnidad, idUnico,pdv) {
    var nroFracciones = $("#cantidad-producto-fraccion-" + codigoProducto +"_"+pdv ).val();
    var nroUnidades = $("#cantidad-producto-unidad-" + codigoProducto  +"_"+pdv).val();
    ...
    if ((nroFracciones * unidadFraccionamiento) >= numeroFracciones) {
        ...
        $("#cantidad-producto-unidad-" + codigoProducto +"_"+pdv ).val(nroUnidades);
        $("#subtotal-producto-unidad-" + codigoProducto +"_"+pdv).html("$" + format(nroUnidades * valorUnidad));
        nroFracciones = (nroFracciones * unidadFraccionamiento) % numeroFracciones;
    }
    $("#subtotal-producto-fraccion-" + codigoProducto +"_"+pdv).html("$" + format(nroFracciones * valorFraccionado));
    $("#cantidad-producto-fraccion-" + codigoProducto +"_"+pdv).val(nroFracciones);
}
...
$(document).on('click', "a[data-role='carrovaciar-entrega']", function() {
    $.ajax({
        ...
        dataType: 'json',
        async: true,
        url: requestUrl + '/puntoventa/venta/ventaasistida/carro/vaciar',
        beforeSend: function() {
            Loading.show();
        },
        complete: function() {
...
function pasoInformacion(actual, siguiente, boton) {
    ...
    $.ajax({
        type: 'POST',
        async: true,
        url: requestUrl + '/puntoventa/venta/ventaasistida/carro/pagar/paso/' + actual + '/post/true',
