...
<?php $idUnico = uniqid() ?>
<?php $position = Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->itemAt($objProducto->codigoProducto);?>
                ...
                <div class="descripciones">
                    ...
                    <div class="" style="color:#A3A3A3;font-size: 16px;">
                        ...
                        <h1 style="color: #ED1C24; font-size: 24px;">
                            <!-- producto agregado -->
                            <a href="" class="itm_ico clst_slct_prod<?php echo (Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->contains($objProducto->codigoProducto) ? " active" : "") ?>" id="icono-producto-agregado-<?php echo $objProducto->codigoProducto ?>">
                        ...
                        <?php foreach($listaSaldos['puntoventa'] as $key => $saldosPDV ):?>
                                <div>
		                                ...
		                                <tr>
		                                    <td valign="bottom" class="container_gray_fracc td-dcto">
		                                        ...
		                                        <div class="row control_cant_detalle">
		                                            <button class="col-md-3 min" style="border:1px solid;" id="disminuir-unidad-<?php echo $objProducto->codigoProducto ?>_<?= $key?>" data-pdv='<?= $key ?>' data-id="<?php echo $idUnico ?>"  data-role="disminuir-cantidad"  data-producto="<?php echo $objProducto->codigoProducto ?>" data-precio="<?= $objPrecio->getPrecio(Precio::PRECIO_UNIDAD) ?>" type="button"><span class="glyphicon glyphicon-minus" ></span></button>
		                                            <div class="col-md-6 select-cantidad"><input id="cantidad-producto-unidad-<?php echo $objProducto->codigoProducto ?>_<?= $key?>" data-pdv='<?= $key ?>' class="increment" type="text" data-role="validar-cantidad-unidad"  data-producto="<?php echo $objProducto->codigoProducto ?>" data-precio="<?= $objPrecio->getPrecio(Precio::PRECIO_UNIDAD) ?>"  maxlength="3" value="0" data-total="700"/></div>
		                                            <button class="col-md-3 min" style="border:1px solid;" id="aumentar-unidad-<?php echo $objProducto->codigoProducto ?>_<?= $key?>" data-pdv='<?= $key ?>'  data-id="<?php echo $idUnico ?>"   data-role="aumentar-cantidad"  data-producto="<?php echo $objProducto->codigoProducto ?>" data-precio="<?= $objPrecio->getPrecio(Precio::PRECIO_UNIDAD) ?>" type="button"><span class="glyphicon glyphicon-plus" ></span></button>
		                                        </div>
		                                    ...
		                                    <td  valign="bottom">
		                                        ...
		                                        <div class="row control_cant_detalle">
		                                            <button class="col-md-3 min" style="border:1px solid;" id="disminuir-fraccion-<?php echo $objProducto->codigoProducto ?>_<?= $key?>" data-pdv="<?= $key?>" onclick="disminuirCantidadFraccionado(<?php echo $objProducto->codigoProducto ?>,<?php echo $objProducto->numeroFracciones ?>,<?php echo $objProducto->unidadFraccionamiento ?>,<?= $objPrecio->getPrecio(Precio::PRECIO_FRACCION) ?>,<?= $objPrecio->getPrecio(Precio::PRECIO_UNIDAD, false) ?>, '<?= $idUnico ?>','<?= $key?>')" type="button"><span  class="glyphicon glyphicon-minus"></span></button>
		                                            <div class="col-md-6 select-cantidad"><input id="cantidad-producto-fraccion-<?php echo $objProducto->codigoProducto ?>_<?= $key?>"  data-pdv="<?= $key?>" data-id="<?php echo $idUnico ?>" class="increment" type="text" onchange="validarCantidadFraccionado(<?php echo $objProducto->codigoProducto ?>,<?php echo $objProducto->numeroFracciones ?>,<?php echo $objProducto->unidadFraccionamiento ?>,<?= $objPrecio->getPrecio(Precio::PRECIO_FRACCION) ?>,<?= $objPrecio->getPrecio(Precio::PRECIO_UNIDAD, false) ?>, '<?= $idUnico ?>','<?= $key?>')" maxlength="3" value="0" data-total="700"/></div>
		                                            <button class="col-md-3 min" style="border:1px solid;" id="aumentar-fraccion-<?php echo $objProducto->codigoProducto ?>_<?= $key?>"  data-pdv="<?= $key?>"  onclick="aumentarCantidadFraccionado(<?php echo $objProducto->codigoProducto ?>,<?php echo $objProducto->numeroFracciones ?>,<?php echo $objProducto->unidadFraccionamiento ?>,<?= $objPrecio->getPrecio(Precio::PRECIO_FRACCION) ?>,<?= $objPrecio->getPrecio(Precio::PRECIO_UNIDAD, false) ?>, '<?= $idUnico ?>','<?= $key?>')" type="button"><span  class="glyphicon glyphicon-plus"></span></button>
		                                        </div>
		                        ...
		                        <div class="" style="margin-top: 13px;">
                                    <?php echo CHtml::link('<div class="btn btn-primary btn-block">Añadir&nbsp;<img src="' . Yii::app()->baseUrl . '/images/desktop/button-carrito.png" alt=""></div>', '#', array('data-producto' => $objProducto->codigoProducto, 'data-cargar-nacional' => 1, 'data-pdv' => $key, 'data-id' => $idUnico, 'data-max' =>  $saldosPDV['Saldos'],'class' => '')); ?><br/>
                                </div>
						<?php endforeach;?>
                        <div class="descripciones">
                        <?php else: ?>
                            ...
                            <?php foreach($listaSaldos['puntoventa'] as $key => $saldosPDV ):?>
                                ...
                                Unidades Agregadas: <?= $position?$position->getUnitPDV($key,true):0 ?>
                                    ...
                                    <div class="detPproduct">
                                        ...
                                        <button class=" min" style="border:1px solid;" id="aumentar_unidad_<?php echo $objProducto->codigoProducto ?>_<?= $key?>" data-role="aumentar-cantidad" data-pdv='<?= $key?>'  data-producto="<?php echo $objProducto->codigoProducto ?>" data-precio="<?= $objPrecio->getPrecio(Precio::PRECIO_UNIDAD) ?>" type="button"><span  class="glyphicon glyphicon-plus"></span></button>
                                        <div class="clear"></div>
                                    </div>
                                    <div class="" style="margin-top: 13px;">
                                    <?php echo CHtml::link('<div class="btn btn-primary btn-block">Añadir&nbsp;<img src="' . Yii::app()->baseUrl . '/images/desktop/button-carrito.png" alt=""></div>', '#', array('data-producto' => $objProducto->codigoProducto, 'data-cargar-nacional' => 1, 'data-pdv' => $key, 'data-id' => $idUnico, 'data-max' =>  $saldosPDV['Saldos'],'class' => '')); ?><br/>
                        ...
                        <?php endif; ?>
                    <?php endif; ?> 
                    <?php if ($this->objCiudadSectorDestino == null): ?>
