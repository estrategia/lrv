        ...
        <td class="btn-pagar" style="text-align:center;">
            ...
            <?php else: ?>
                <div class="group-botones-cantidad">
                    <div class="ressete">
                        <input class="increment btn-xs vitalcall" data-position="<?php echo $position->getId(); ?>" type="text" value="<?php echo $position->getQuantityUnit() ?>" id="cantidad-producto-unidad-<?php echo $position->getId() ?>" placeholder="0"/>
                    </div>
                </div>
            <?php endif; ?>

            <?php if (!$lectura): ?>
				<?php echo CHtml::link('Modificar cantidades', CController::createUrl('/puntoventa/venta/ventaasistida/catalogo/producto', array('producto' => $position->getObjProducto()->codigoProducto, 'descripcion' => $position->getObjProducto()->getCadenaUrl())), array('class' => 'btn btn-primary btn-xs')); ?>
            <?php endif; ?>
        </td>
        <td class="text-right">
            ...
            <?php if (Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->getObjCiudadDestino()->excentoImpuestos == 0 && $position->getTax() > 0): ?>
                <br>
