...
/**
 * Description of FormaPagoVentaAsitidaForm
 *
 * @author miguel.sanchez
 */
class FormaPagoVentaAsistidaForm extends CFormModel {
    ...
    public function rules() {
        ...
        $rules = array();
//         $rules[] = array('cedulaRemitente', 'required', 'message' => '{attribute} no puede estar vacío');
...
//         $rules[] = array('tipoEntrega', 'tipoEntregaValidate');
    ...
    public function seleccionarPdv($indicePdv) {
        if (!empty($this->listPuntosVenta)) {
            ...
            foreach ($puntoVenta[4] as $indiceProd => $producto) {
                $position = Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->getItemAtCode($producto->CODIGO_PRODUCTO);

                if ($position !== null) {
                    $arrPositions[$producto->CODIGO_PRODUCTO] = $producto->CODIGO_PRODUCTO;
                    if ($producto->SALDO_UNIDAD >= $producto->CANTIDAD_UNIDAD) {
                        Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->update($position, false, $producto->CANTIDAD_UNIDAD);
                    } else {
                        Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->update($position, false, $producto->SALDO_UNIDAD);
                    }

                    if ($producto->SALDO_FRACCION >= $producto->CANTIDAD_FRACCION) {
                        Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->update($position, true, $producto->CANTIDAD_FRACCION);
                    } else {
                        Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->update($position, true, $producto->SALDO_FRACCION);
                    }
                }
            }

            foreach (Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->getPositions() as $position) {
                if (!isset($arrPositions[$position->getObjProducto()->codigoProducto])) {
                    Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->remove($position->getId());
                }
            }
        }
    }
    ...
    public function tipoEntregaValidate($attribute, $params) {
        /*if ($this->tipoEntrega == Yii::app()->params->entrega['tipo']['presencial'] && Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->getStoredItemsCount() > 0) {
    ...
    public function calcularFormulas($positions){
        $array = array();
    	if (isset(Yii::app()->session[Yii::app()->params->shoppingCartNationalSale['sesion']['formulaMedica']])) {
            $array = Yii::app()->session[Yii::app()->params->shoppingCartNationalSale['sesion']['formulaMedica']];
    	}
        ...
        Yii::app()->session[Yii::app()->params->shoppingCartNationalSale['sesion']['formulaMedica']] = $array;
    }
