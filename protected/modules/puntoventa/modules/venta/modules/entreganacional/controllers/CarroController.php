...
// 		}
		...
		if ($cantidadCarroUnidad + $cantidadCarroFraccion + $cantidadU + $cantidadF*$divisionFracciones <= $maximo) {
			if ($cantidadU >= 0) {
				...
				Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->put($objProductoCarro, false, $cantidadU);
			}
			if ($cantidadF >= 0) {
				$objProducto->saldosDisponibles = $maximo;
				$objProductoCarro = new ProductoCarro($objProducto);
				Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->put($objProductoCarro, true, $cantidadF);
			}
		} else {
	...
	public function actionPagar($paso = null, $post = false) {
		...
		if (Yii::app()->getModule('puntoventa')->shoppingCartNationalSale->isEmpty()) {
			if ($post) {
				...
				$this->redirect($this->createUrl('/puntoventa/venta/entreganacional/carro'));
			}
			Yii::app()->end();
		}
				...
				switch ($paso) {
					case Yii::app()->params->entregaNacional['pagar']['pasos'][1]:
						...
						if ($form->validate()) {
							...
							echo CJSON::encode(array('result' => 'ok', 'response' => 'Datos guardados', 'redirect' => $this->createUrl("/puntoventa/venta/entreganacional/carro/pagar", array('paso' => $siguiente))));
							Yii::app()->end();
						} else {
					...
					case Yii::app()->params->entregaNacional['pagar']['pasos'][2]:
						...
						if ($form->validate()) {
							...
							Yii::app()->session[Yii::app()->params->entregaNacional['sesion']['carroPagarForm']] = $modelPago;
							echo CJSON::encode(array('result' => 'ok', 'response' => "Datos guardados", 'redirect' => $this->createUrl("/puntoventa/venta/entreganacional/carro/comprar")));
							Yii::app()->end();
						} else {
							echo CActiveForm::validate($form);
							Yii::app()->end();
	...
	public function actionComprar() {
		...
		if (isset(Yii::app()->session[Yii::app()->params->entregaNacional['sesion']['carroPagarForm']]) && Yii::app()->session[Yii::app()->params->entregaNacional['sesion']['carroPagarForm']] != null)
			...
			if ($pasoValidacion != null) {
				...
				$this->redirect($this->createUrl('/puntoventa/venta/entreganacional/carro/pagar', $paramsValidacion));
				Yii::app()->end();
			}
			...
			if ($resultCompra['result'] == 1) {
				...
				$this->redirect($this->createUrl('/puntoventa/venta/entreganacional/carro/pagar'));
				Yii::app()->end();
			}
	}
