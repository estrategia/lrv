...
// 		}
	...
	public function actionModificar() {
		...
		// !Yii::app()->shoppingCartNationalSale->contains($id)
	...
	public function actionPagar($paso = null, $post = false) {
		...
		if (Yii::app()->shoppingCartNationalSale->isEmpty()) {
			if ($post) {
				echo CJSON::encode(array('result' => 'ok', 'response' => 'Carro vac&iacute;o', 'redirect' => $this->createUrl("/puntoventa/entreganacional/carro")));
			} else {
				$this->redirect($this->createUrl('/puntoventa/entreganacional/carro'));
			}
			Yii::app()->end();
		}
			...
			if ($post) {
				...
				switch ($paso) {
					case Yii::app()->params->entregaNacional['pagar']['pasos'][1]:
						$form = new FormaPagoEntregaNacionalForm($paso);
						...
						if (isset($_POST['FormaPagoEntregaNacionalForm'])) {
							foreach ($_POST['FormaPagoEntregaNacionalForm'] as $atributo => $valor) {
						...
						if ($form->validate()) {
							...
							echo CJSON::encode(array('result' => 'ok', 'response' => 'Datos guardados', 'redirect' => $this->createUrl("/puntoventa/entreganacional/carro/pagar", array('paso' => $siguiente))));
							Yii::app()->end();
						} else {
					...
					case Yii::app()->params->entregaNacional['pagar']['pasos'][2]:
						...
						if ($form->validate()) {
							...
							Yii::app()->session[Yii::app()->params->entregaNacional['sesion']['carroPagarForm']] = $modelPago;
							echo CJSON::encode(array('result' => 'ok', 'response' => "Datos guardados", 'redirect' => $this->createUrl("/puntoventa/entreganacional/carro/comprar")));
							Yii::app()->end();
						} else {
							echo CActiveForm::validate($form);
							Yii::app()->end();
				...
				switch ($paso) {
					...
					case Yii::app()->params->entregaNacional['pagar']['pasos'][2]:
						...
						$modelPago->calcularConfirmacion(Yii::app()->shoppingCartNationalSale->getPositions());
						//$modelPago->calcularFormulas(Yii::app()->shoppingCartNationalSale->getPositions());
						break;
				}
	...
	public function actionComprar() {
		...
		if (isset(Yii::app()->session[Yii::app()->params->entregaNacional['sesion']['carroPagarForm']]) && Yii::app()->session[Yii::app()->params->entregaNacional['sesion']['carroPagarForm']] != null)
			...
			$modelPago->totalCompra = Yii::app()->shoppingCartNationalSale->getTotalCost();
			...
			$modelPago->validarConfirmacion(Yii::app()->shoppingCartNationalSale->getPositions());
			...
			if ($pasoValidacion != null) {
				...
				$this->redirect($this->createUrl('/puntoventa/entreganacional/carro/pagar', $paramsValidacion));
				Yii::app()->end();
			}
			...
			if ($resultCompra['result'] == 1) {
				...
				$this->redirect($this->createUrl('/puntoventa/entreganacional/carro/pagar'));
				Yii::app()->end();
			}
	}
	
	private function procesoCompra(FormaPagoEntregaNacionalForm $modelPago, $tipoEntrega) {
		...
		try {
			...
			if($modelPago->recogida == 1){
				...
				// guardar la compra como remitida POS y llamar el webservice de remisi�n.
			...
			$objCompra->codigoPerfil = Yii::app()->shoppingCartNationalSale->getCodigoPerfil();
			...
			$objCompra->subtotalCompra = Yii::app()->shoppingCartNationalSale->getCostToken();
			$objCompra->impuestosCompra = Yii::app()->shoppingCartNationalSale->getTaxPrice();
			$objCompra->baseImpuestosCompra = Yii::app()->shoppingCartNationalSale->getBaseTaxPrice();
			$objCompra->domicilio = Yii::app()->shoppingCartNationalSale->getShipping();
			$objCompra->flete = Yii::app()->shoppingCartNationalSale->getExtraShipping();
			$objCompra->totalCompra = Yii::app()->shoppingCartNationalSale->getTotalCost();
			...
			$objFormasPago->valor = Yii::app()->shoppingCartNationalSale->getTotalCost();
		...
		/*	} else if ($tipoEntrega == Yii::app()->params->entrega['tipo']['presencial']) {
			...
			//items de compra
			$positions = Yii::app()->shoppingCartNationalSale->getPositions();
