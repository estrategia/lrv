...
//         $rules[] = array('tipoEntrega', 'tipoEntregaValidate');
    ...
    public function seleccionarPdv($indicePdv) {
        if (!empty($this->listPuntosVenta)) {
            ...
            foreach ($puntoVenta[4] as $indiceProd => $producto) {
                $position = Yii::app()->shoppingCartNationalSale->getItemAtCode($producto->CODIGO_PRODUCTO);

                if ($position !== null) {
                    $arrPositions[$producto->CODIGO_PRODUCTO] = $producto->CODIGO_PRODUCTO;
                    if ($producto->SALDO_UNIDAD >= $producto->CANTIDAD_UNIDAD) {
                        Yii::app()->shoppingCartNationalSale->update($position, false, $producto->CANTIDAD_UNIDAD);
                    } else {
                        Yii::app()->shoppingCartNationalSale->update($position, false, $producto->SALDO_UNIDAD);
                    }

                    if ($producto->SALDO_FRACCION >= $producto->CANTIDAD_FRACCION) {
                        Yii::app()->shoppingCartNationalSale->update($position, true, $producto->CANTIDAD_FRACCION);
                    } else {
                        Yii::app()->shoppingCartNationalSale->update($position, true, $producto->SALDO_FRACCION);
                    }
                }
            }

            foreach (Yii::app()->shoppingCartNationalSale->getPositions() as $position) {
                if (!isset($arrPositions[$position->getObjProducto()->codigoProducto])) {
                    Yii::app()->shoppingCartNationalSale->remove($position->getId());
                }
            }
        }
    }
    ...
    public function tipoEntregaValidate($attribute, $params) {
        /*if ($this->tipoEntrega == Yii::app()->params->entrega['tipo']['presencial'] && Yii::app()->shoppingCartNationalSale->getStoredItemsCount() > 0) {
