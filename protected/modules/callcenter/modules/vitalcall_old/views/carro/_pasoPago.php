...
<?php foreach ($listFormaPago as $idx => $objFormaPago): ?>
    ...
    <div data-role="formapago" data-tipo="<?php echo $objFormaPago->idFormaPago ?>">
        <input type="radio" name="FormaPagoVitalCallForm[idFormaPago]" id="FormaPagoVitalCallForm_idFormaPago_<?php echo $objFormaPago->idFormaPago ?>" value="<?php echo $objFormaPago->idFormaPago ?>" <?php echo ($objFormaPago->idFormaPago == $modelPago->idFormaPago ? "checked" : "") ?>> 
        <label for="FormaPagoVitalCallForm_idFormaPago_<?php echo $objFormaPago->idFormaPago ?>"><?php echo $objFormaPago->formaPago ?></label>
    </div>
<?php endforeach; ?>
<?php if (!empty_lrv($listDatafono)): ?>
    <div data-role="formapago" data-tipo="datafono">
        <input type="radio" name="FormaPagoVitalCallForm[idFormaPago]" id="FormaPagoVitalCallForm_idFormaPago_datafono" value="<?php echo (in_array($modelPago->idFormaPago, Yii::app()->params->formaPago['tarjetasDatafono']) ? $modelPago->idFormaPago : "" ) ?>" <?php echo (in_array($modelPago->idFormaPago, Yii::app()->params->formaPago['tarjetasDatafono']) ? "checked" : "" ) ?> >
            ...
            <?php if ($objFormaPago->idFormaPago == Yii::app()->params->formaPago['idCredirebaja']): ?>
                <div data-role="formapago-descripcion" data-tipo="<?php echo $objFormaPago->idFormaPago ?>" class="border-gray row center-div<?php echo ($modelPago->idFormaPago == $objFormaPago->idFormaPago ? "" : " display-none" ) ?>">
                    ...
                    <div class="col-md-4 info-oficina">
                        ...
                        <?php echo $form->dropDownList($modelPago, 'cuotasTarjeta', FormaPagoVitalCallForm::listDataCuotas(), array('class' => 'form-control select', 'placeholder' => '0', 'style' => "border-radius:0px;")); ?>
