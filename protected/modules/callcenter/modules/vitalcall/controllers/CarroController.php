...
class CarroController extends ControllerVitalcall {
	...
	public function actionAgregar() {
		...
		echo CJSON::encode ( array (
				'result' => 'error',
				'response' => 'Solicitud inválida, tipo incorrecto' 
		) );
		Yii::app ()->end ();
	}
	public function agregarVitalCall(FormaPagoVitalCallForm $modelPago) {
		...
		if ($objProductoFormula === null) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'ProductoFormula no existente' 
			) );
			Yii::app ()->end ();
		}
		...
		if (! $objProductoFormula->objProductoVC->esVigente ()) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => "Producto no vigente [vitalcall: $objProductoFormula->idProductoVitalCall, producto: " . $objProductoFormula->objProductoVC->codigoProducto . "]" 
			) );
			Yii::app ()->end ();
		}
		...
		$objProducto = Producto::model ()->find ( array (
				'with' => array (
						'listSaldos' => array (
								'condition' => '(listSaldos.codigoCiudad=:ciudad AND listSaldos.codigoSector=:sector) OR listSaldos.idProductoSaldos IS NULL' 
						),
						'listPrecios' => array (
								'condition' => '(listPrecios.codigoCiudad=:ciudad AND listPrecios.codigoSector=:sector) OR listPrecios.idProductoPrecios IS NULL' 
						) 
				),
		...
		echo CJSON::encode ( array (
				...
				'response' => array (
						...
						'mensajeHTML' => $this->renderPartial ( '_carroAgregado', array (
		...
		if (Yii::app ()->shoppingCartVitalCall->isEmpty ()) {
			if ($post) {
				echo CJSON::encode ( array (
						...
						'redirect' => $this->createUrl ( "/callcenter/vitalcall/carro" ) 
				) );
				Yii::app ()->end ();
			} else {
		...
		if ($post) {
				...
				case Yii::app ()->params->vitalCall ['pagar'] ['pasos'] [1] :
					...
					if ($form->validate ()) {
						...
						echo CJSON::encode ( array (
								...
								'redirect' => $this->createUrl ( "/callcenter/vitalcall/carro/pagar", array (
										'paso' => $siguiente 
								) ) 
						) );
						Yii::app ()->end ();
				...
				case Yii::app ()->params->vitalCall ['pagar'] ['pasos'] [2] :
					...
					if ($form->validate ()) {
						...
						echo CJSON::encode ( array (
								'result' => 'ok',
								'response' => "Datos guardados",
								'redirect' => $this->createUrl ( "/callcenter/vitalcall/carro/comprar" ) 
						) );
						Yii::app ()->end ();
					} else {
						echo CActiveForm::validate ( $form );
			...
			switch ($paso) {
				...
				case Yii::app ()->params->vitalCall ['pagar'] ['pasos'] [2] :
					...
					$arrayFormula = array ();
					foreach ( Yii::app ()->shoppingCartVitalCall->getPositions () as $key => $position ) {
						if ($position->isFormula ()) {
							$array ["form_" . $position->objProductoFormula->objFormulaVC->idFormula] = array (
	...
	private function procesoCompra(FormaPagoVitalCallForm $modelPago, $tipoEntrega) {
		...
		try {
			...
			try {
				...
				$result = array (
						0 => 1,
						1 => 'congelar prueba ok',
						2 => 'miguel.sanchex@gmail.com.co' 
				);
