...
class PedidoController extends ControllerOperator {
    ...
    public function filterBusqueda($filter){
        if(empty(Yii::app()->session[Yii::app()->params->entreganacional['sesion']['pdvDestino']])){
    ...
    public function actionAsignarpdv() {
        ...
        Yii::app()->session[Yii::app()->params->entreganacional['sesion']['pdvDestino']] = $objPdv;
    ...
    public function actionBuscarProducto() {
        ...
        $pdvDestino = Yii::app()->session[Yii::app()->params->entreganacional['sesion']['pdvDestino']];
        $pdvOrigen = PuntoVenta::model()->find(
            array(
                'condition' => "IDComercial =:idcomercial", 
                "params" => array(
                    ":idcomercial" => Yii::app()->session[Yii::app()->params->entreganacional['sesion']['pdv']])
                ));
        if($pdvOrigen === null){
            throw new CHttpException(404, 'Solicitud inválida.');
        }
