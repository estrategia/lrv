...
class UserSubasta {
    ...
    public $loginUrl = array('/subasta/usuario/autenticar');
    public $homeUrl = array('/subasta');
    public $profile = null;
    ...
    public function restoreFromSession() {
        ...
        if (isset(Yii::app()->session[Yii::app()->params->subasta['sesion']['usuario']]) && (Yii::app()->session[Yii::app()->params->subasta['sesion']['usuario']] instanceof Operador)) {
            $user = Yii::app()->session[Yii::app()->params->subasta['sesion']['usuario']];
        } else {
            Yii::app()->session[Yii::app()->params->subasta['sesion']['usuario']] = null;
        }
    ...
    public function login($identity) {
        if ($identity !== null && ($identity instanceof OperatorIdentity) && $identity->user !== null) {
            Yii::app()->session[Yii::app()->params->subasta['sesion']['usuario']] = $identity->user;


            $pdv = OperadorSubasta::model()->find('idOperador =:idoperador', array(':idoperador' => $identity->user->idOperador));

            if ($pdv != null) {
                Yii::app()->session[Yii::app()->params->subasta['sesion']['pdv']] = $pdv->idComercial;
                ...
                if ($objCiudadSector != null) {
                    Yii::app()->session[Yii::app()->params->subasta['sesion']['objCiudadSector']] = $objCiudadSector;
                }
            }
    ...
    public function logout() {
        $sessions = Yii::app()->params->subasta['sesion'];
