...
class CarroController extends ControllerVendedor {
	...
	public function filterLoginajax($filter) {
		if (! Yii::app ()->controller->module->user->getClienteLogueado ()) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'No se detecta usuario autenticado, por favor iniciar sesión para continuar' 
			) );
			Yii::app ()->end ();
		}
		$filter->run ();
	...
	public function actionPasoporel() {
		...
		if ($modelPago == null) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Proceso de compra no iniciado' 
			) );
			Yii::app ()->end ();
		}
		...
		if ($opcion == "modal") {
			...
			echo CJSON::encode ( array (
					'result' => 'ok',
					'response' => $this->renderPartial ( "_pagarPresencial", array (
							'listPuntosVenta' => $modelPago->listPuntosVenta 
					), true ) 
			) );
			Yii::app ()->end ();
			...
			if ($modelPago->listPuntosVenta [0] == 0) {
				echo CJSON::encode ( array (
						'result' => 'error',
						'response' => $modelPago->listPuntosVenta [1] 
				) );
				Yii::app ()->end ();
			}
			...
			if ($idx == null) {
				echo CJSON::encode ( array (
						'result' => 'error',
						'response' => 'Datos inv&aacute;lidos' 
				) );
				Yii::app ()->end ();
			}
			...
			echo CJSON::encode ( array (
					'result' => 'ok',
					'response' => 'Punto de venta seleccionado' 
			) );
			Yii::app ()->end ();
		} else {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Solicitud env&aacute;lida' 
			) );
			Yii::app ()->end ();
		}
	}
	public function actionAgregar() {
		...
		if ($producto === null || $cantidadU === null || $cantidadF === null) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Solicitud inválida, no se detectan datos' 
			) );
			Yii::app ()->end ();
		}
	...
	public function actionAgregarcompra() {
		if ($this->objSectorCiudad === null) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Para agregar este pedido al carro de compras, debes seleccionar ciudad.' 
			) );
			Yii::app ()->end ();
		}
		...
		if ($nUnidadesCarro == 0) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Productos no disponibles' 
			) );
			Yii::app ()->end ();
		}
		...
		echo CJSON::encode ( array (
				...
				'response' => array (
						'canastaHTML' => $this->renderPartial ( $canasta, null, true ),
						'mensajeHTML' => $this->renderPartial ( '//common/mensajeHtml', array (
								'mensaje' => "$porcentajeCarro% de lista agregada" 
						), true ) 
				) 
		) );
		Yii::app ()->end ();
	...
	public function actionAgregarcotizacion() {
		...
		if ($objCotizacion === null) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Cotizaci&oacute;n no existente o vencida' 
			) );
			Yii::app ()->end ();
		}
	...
	public function actionAgregarlista() {
		...
		if ($objSectorCiudad === null) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Seleccionar ubicación.' 
			) );
			Yii::app ()->end ();
		}
	...
	public function actionAgregarBodega() {
		...
		if (! $objPagoForm->tieneDomicilio ( $objSectorCiudad )) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => "La cantidad solicitada no está disponible en este momento." 
			) );
			Yii::app ()->end ();
		}
	...
	public function actionEliminar() {
		...
		if ($position == null) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Producto no agregado a carro' 
			) );
			Yii::app ()->end ();
		}
	...
	public function actionPagar($paso = null, $post = false) {
		...
		if ($this->objSectorCiudad === null) {
			if ($post) {
				echo CJSON::encode ( array (
						'result' => 'ok',
						'response' => 'Seleccionar ubicaci&oacute;n',
						'redirect' => $this->createUrl ( '/vendedor/sitio/ubicacion' ) 
				) );
				Yii::app ()->end ();
			} else {
		...
		if (Yii::app ()->shoppingCartSalesman->isEmpty ()) {
			...
			if ($post) {
				echo CJSON::encode ( array (
						'result' => 'ok',
						'response' => 'Carro vac&iacute;o',
						'redirect' => $this->createUrl ( "/vendedor/carro/pagar" ) 
				) );
				Yii::app ()->end ();
			} else {
		...
		if (! Yii::app ()->controller->module->user->getClienteLogueado () && ! Yii::app ()->controller->module->user->getIsClienteInvitado () && $modelPago == null) {
			...
			$this->render ( '/vendedor/usuario/autenticar', array (
					'pagar' => true 
			) );
			Yii::app ()->end ();
		}
				...
				case Yii::app ()->params->pagar ['pasos'] [1] :
					...
					if ($form->validate ()) {
						...
						echo CJSON::encode ( array (
								...
								'redirect' => $this->createUrl ( "carro/pagar", array (
										'paso' => $siguiente 
								) ) 
						) );
						Yii::app ()->end ();
						...
						if (isset ( $_POST ['FormaPagoVendedorForm'] )) {
							...
							if ($form->validate ()) {
								...
								echo CJSON::encode ( array (
										'result' => 'ok',
										'response' => "Datos guardados",
										'redirect' => $this->createUrl ( "carro/comprar" ) 
								) );
								Yii::app ()->end ();
							} else {
								echo CActiveForm::validate ( $form );
	...
	public function actionComprar() {
		...
		if ($modelPago->idFormaPago == Yii::app ()->params->formaPago ['pasarela'] ['idPasarela']) {
			$this->render ( 'pasarela', array (
					'pagoExpress' => $modelPago->pagoExpress,
					'isMobil' => true 
			) );
			Yii::app ()->end ();
		}
	...
	private function procesoCompra(FormaPagoVendedorForm $modelPago, $tipoEntrega) {
		...
		try {
			...
			if (isset ( Yii::app ()->session [Yii::app ()->params->vendedor ['sesion'] ['formulaMedica']] )) {
				foreach ( Yii::app ()->session [Yii::app ()->params->vendedor ['sesion'] ['formulaMedica']] as $formula ) {
					...
					if (! $formulaMedica->save ()) {
						echo "<pre>";
						print_r ( $formulaMedica->getErrors () );
						exit ();
			...
			if ($modelPago->idFormaPago != Yii::app ()->params->formaPago ['pasarela'] ['idPasarela']) {
				...
				try {
					$result = $client->__soapCall ( "CongelarCompraAutomatica", array (
							'idPedido' => $objCompra->idCompra 
					) ); // 763759, 763743
	...
	public function actionAdicionarFormula() {
		...
		$array [] = array (
				'nombreMedico' => $_POST ['FormulasMedicas'] ['nombreMedico'],
	...
	public function actionUsarCodigo() {
		...
		if ($bonoTienda === null) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Bono no disponible.' 
			) );
			Yii::app ()->end ();
		}
		
		// revisar si ese bono ya est� cargado en sesi�n.
		
		if ($modelPago->verificarPromocional ()) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Ya se est&aacute; utilizando un c&oacute;digo promocional en la compra.' 
			) );
			Yii::app ()->end ();
		}
		
		// revisar si el bono ya fu� utilizado por la persona en otra compra.
		...
		if ($comprobarBono) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'El bono ya fu&eacute utilizado en otra compra.' 
			) );
			Yii::app ()->end ();
		}
		...
		if ($bonoTienda->cantidadUso < 1) {
			echo CJSON::encode ( array (
					'result' => 'error',
					'response' => 'Ya se agotaron las cantidades disponibles para este bono.' 
			) );
			Yii::app ()->end ();
		}
		// revisar si ese bono ya est� cargado en sesi�n.
		...
		// Si validamos lo anterior y el uso se puede dar, mostrar informaci�n del descuento del bono.
		
		echo CJSON::encode ( array (
				'result' => 'ok',
				'response' => 'Bono disponible por ' . Yii::app ()->numberFormatter->format ( Yii::app ()->params->formatoMoneda ['patron'], $bonoTienda->valorBono, Yii::app ()->params->formatoMoneda ['moneda'] ) . "\nDesea Utilizarlo?",
				'bono' => $bonoTienda->idBonoTiendaTipo 
		) );
	}
