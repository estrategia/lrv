        ...
        if ($cantidadU > 0) {
            ...
            if (isset($objSaldo) && $cantidadCarroUnidad + $cantidadU <= $objSaldo->saldoUnidad) {
               ...
               /* $objSaldoBodega = ProductosSaldosCedi::model()->find(array(
				...
				$query->bindParam(":cedi", implode(",",$this->bodegas), PDO::PARAM_STR);
            ...
            foreach ($positions as $position) {
                if ($position->isProduct()) {
                    ...
                    if ($position->getQuantityStored() > 0) {
                    	...
                    	foreach($position->calculateUnidadesBodega($this->bodegas)['cantidades'] as $codigoSucursal => $cantidadBodega){
                    		$objSaldoBodega = ProductosSaldosCedi::model()->find(array(
